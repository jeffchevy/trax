...
<html>
	<head>
		...
		<style type="text/css">
			...
			th.rotateheader > div {
			  transform: translate(0px, 51px) rotate(-25deg);
			  -webkit-transform: translate(0px, 51px) rotate(-25deg);
			  width: 30px;
			  color: white;
			}
	...
	<div id="instruct">
<security:authorize  access="hasRole('ROLE_LEADER')">
	To add a course date, click on a table cell and select the appropriate date.<br>
</security:authorize>
		To see a description of the course hold the mouse cursor over the course title. </div>
	<div class="table">					
		<div class="row">
					...
					<table id="leaderList" cellspacing="0" class="dataTable" summary="Leaders">
						...
						<thead>
							<tr>
								<th>Leader Name</th>
								<c:forEach var="course" items="${courses}">
								 <th class="rotateheader" title="${course.description}"><div>${course.name}</div></th>
								</c:forEach>
							</tr>
						</thead>
						<tfoot>
						<tr>
								...
								<c:forEach var="course" items="${courses}">
								 <td title=${course.description}><br>
